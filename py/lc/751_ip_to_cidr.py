"""
https://leetcode.com/problems/ip-to-cidr/description/?envType=company&envId=databricks&favoriteSlug=databricks-all
...
class Solution:
    def ip_to_cidr(self, ip: str, n: int) -> List[str]:
        ...
        while n:
            if ip_as_int == 0:
                ...
                alignment_bits = (ip_as_int & -ip_as_int).bit_length() - 1
            mask = max(32 - alignment_bits, 32 - (n.bit_length() - 1))
            ans.append(self._int_to_ip(ip_as_int) + "/" + str(mask))
            ip_as_int += 1 << (32 - mask)
            n -= 1 << (32 - mask)
    ...
    def _int_to_ip(self, x: int) -> str:
        return ".".join(str((x >> i) % 256) for i in (24, 16, 8, 0))


def test_one():
    solution = Solution()
    result = solution.ip_to_cidr("255.0.0.7", 10)

    assert result == ["255.0.0.7/32", "255.0.0.8/29", "255.0.0.16/32"]


def test_two():
    solution = Solution()
    result = solution.ip_to_cidr("117.145.102.62", 8)

    assert result == ["117.145.102.62/31", "117.145.102.64/30", "117.145.102.68/31"]


def test_three():
    solution = Solution()
    result = solution.ip_to_cidr("0.0.0.0", 1)
